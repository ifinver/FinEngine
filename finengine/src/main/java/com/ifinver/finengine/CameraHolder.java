...
public class CameraHolder {
    ...
    public static int CAMERA_START = 1;
    public static int CAMERA_INITED = 2;
    ...
    public int start(int expectedWidth, int expectedHeight, Context ctx, int cameraId, CameraListener listener) {
        if (mEngineThread.mCamera == null || mEngineThread.exited) {
            this.mAppCtx = ctx.getApplicationContext();
            Log.d(TAG, " mAppCtx = null ? " + (mAppCtx));
            mEngineThread.start(expectedWidth, expectedHeight, cameraId, listener);
            return CAMERA_START;
        } else {
            Log.e(TAG, "初始化摄像头时,摄像头已经初始化过了！");
            return CAMERA_INITED;
    ...
    private class FinEngineThread extends HandlerThread implements Handler.Callback, Camera.PreviewCallback {
        ...
        private final int MSG_OPEN_FLASH_LIGHT = 0x106;
        ...
        private volatile int mRequestCameraId = -1;
...
//            Log.d(TAG, "=================onPreviewFrame========================= exited = ");
            ...
            if (!exited) {
                if (mListener != null) {
                    mListener.onVideoBuffer(data, mFrameWidth, mFrameHeight, mCameraOrientation,
                            mRequestCameraId == -1 ? isFrontCurrent() : isRequestCameraIdFrontCurrent());
                }
            }
        }
        ...
        public boolean isRequestCameraIdFrontCurrent() {
            return mRequestCameraId == Camera.CameraInfo.CAMERA_FACING_FRONT;
        }
        ...
        public void start(int expectedWidth, int expectedHeight, int cameraId, CameraListener listener) {
            this.mExpectedWidth = expectedWidth;
            this.mExpectedHeight = expectedHeight;
            this.mRequestCameraId = cameraId;
            this.mListener = listener;
            Message msg = mSelfHandler.obtainMessage();
            msg.what = MSG_INIT;
            msg.obj = cameraId;
            if (mSelfHandler != null) {
        ...
        public void openFlashLight(boolean isOpen) {
            ...
            synchronized (FinEngineThread.class) {
                if (mSelfHandler != null) {
                    Message message = Message.obtain();
                    message.what = MSG_OPEN_FLASH_LIGHT;
                    message.obj = isOpen;
                    mSelfHandler.sendMessage(message);
                }
            }
...
//            Log.d(TAG, "openFlashLight");
//            if (mCameraIndex == cameraId || mRequestCameraId == -1) {
//                return;
//            }
//            mRequestCameraId = cameraId;
//        }
        ...
        private void toggleCameraInternal() {
            ...
            final boolean finalSuccess = initCamera(null);
            if (mListener != null) {
        ...
        public boolean initCamera(Message msg) {
            Log.w(TAG, "摄像头开始初始化");
            boolean init = false;
            synchronized (this) {
                if (mCamera != null) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    try {
                        if (msg != null && msg.obj != null) {
                            mRequestCameraId = (int)msg.obj;
                            mCamera = Camera.open(mRequestCameraId);
                            Log.w(TAG, "mRequestCameraId = " + mRequestCameraId);
                        } else {
...
//                    }
        ...
        public void setCameraDegreeByWindowRotation(int windowRotation) {
            ...
            if (mCamera != null) {
                ...
                if (mRequestCameraId != -1) {
                    Camera.getCameraInfo(mRequestCameraId, info);
                } else {
